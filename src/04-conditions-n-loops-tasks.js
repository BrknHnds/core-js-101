...
function getFizzBuzz(num) {
  ...
  if (fizzBuzz(num)) return 'FizzBuzz';
...
function isInsideCircle(circle, point) {
  ...
  return (x - circle.center.x) ** 2 + (y - circle.center.y) ** 2 < r ** 2;
}
...
function findFirstSingleChar(str) {
  const strArr = str.split(' ').map((e) => e.split('')).flat();
  const tempArr = [];
...
function isCreditCardNumber(ccn) {
  const ccnArr = String(ccn).split('').map((e) => +e);
  for (let i = ccnArr.length - 2; i >= 0; i -= 2) {
    ccnArr[i] += ccnArr[i];
    if (ccnArr[i] >= 10) ccnArr[i] -= 9;
  }
  return ccnArr.reduce((acc, cv) => {
    const sum = acc + cv;
    return sum;
  }, 0) % 10 === 0;
}
